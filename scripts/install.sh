#!/usr/bin/env bash
# Instalador de MediCopilot (despliegue self-hosted autocontenido).
#
...
# sola vez. Todo lo demás (correo, Google Drive, respaldos, proveedores de IA) se
# configura DESDE LA UI, autenticado y auditado — sin editar archivos.
#
...
# La política (registro, correo, respaldos, IA) se administra desde la UI.
...
SMTP_PASSWORD=
SMTP_FROM_EMAIL=
SMTP_FROM_NAME=MediCopilot
SMTP_TLS=true
SMTP_SSL=false
SMTP_USE_CREDENTIALS=true

# Puente interno backend <-> model-gateway (mismos valores en ambos servicios).
AGENT_GATEWAY_TICKET_SECRET=$(rand_hex)
AGENT_GATEWAY_INTERNAL_SECRET=$(rand_hex)
ENV
...
echo " correo, dominio verificado, Google Drive/respaldos y tu proveedor"
echo " de IA — todo desde la interfaz, sin volver a tocar archivos."
echo "=============================================================="
