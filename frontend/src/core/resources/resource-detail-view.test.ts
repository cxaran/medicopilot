...
test("fieldDisplayKind: campo FK (nombre resuelve a recurso) se pinta como relación", () => {
  assert.equal(fieldDisplayKind(field({ name: "patient_id", widget: "text" })), "relation");
  assert.equal(fieldDisplayKind(field({ name: "doctor_id", widget: "text" })), "relation");
  assert.equal(fieldDisplayKind(field({ name: "consultation_id", widget: "text" })), "relation");
  // FK también detectable cuando viene de la lista (sin widget) con type uuid.
  assert.equal(fieldDisplayKind(field({ name: "patient_id", widget: null, type: "uuid" })), "relation");
...
test("formatDisplayValue: relación cae a su UUID (la etiqueta se resuelve en el componente)", () => {
  const fk = field({ name: "patient_id", widget: "text" });
...
test("displayFields: une create ∪ update para mostrar la FK INMUTABLE (que update omite)", () => {
  const capability = {
    forms: {
      // patient_id es inmutable: sólo está en creación. Debe verse en el detalle.
      create: {
        fields: [
          { name: "patient_id", label: "Paciente", type: "uuid", required: true, editable: true, widget: "text" },
  ...
  assert.deepEqual(fields.map((f) => f.name), ["patient_id", "title", "status"]);
