...
// Mapa nombre-de-campo -> recurso destino. Cubre las FK clínicas comunes (paciente,
// médico, consulta). ``attending_doctor_id`` y ``doctor_id`` apuntan ambos a ``doctors``.
const RELATION_TARGETS: Readonly<Record<string, Omit<RelationTarget, "field">>> = {
  patient_id: {
    resource: "patients",
    labelFields: ["full_name"],
    secondaryFields: ["record_number", "curp"],
  },
  doctor_id: {
    resource: "doctors",
    labelFields: ["professional_name"],
    secondaryFields: ["specialty", "professional_license_number"],
  },
  attending_doctor_id: {
    resource: "doctors",
    labelFields: ["professional_name"],
