 ...
 *      se RESUMEN/ELIDEN los intercambios más viejos SIN perder identificadores clínicos. La
 *      compactación afecta SOLO la ventana que ve el modelo; NO toca el almacén de registro
 *      (los datos en FastAPI son la autoridad y quedan intactos; este módulo es PURO, no hace
 *      red). Política de preservación: nunca se resume un identificador (UUID de paciente/
 *      consulta/receta/cita), los planes APROBADOS se conservan verbatim (segmentos
...
/** Extrae identificadores clínicos (UUID) únicos del texto, en orden de aparición. */
...
export interface CompactionResult {
  ...
  recap: string | null;
  /** Identificadores clínicos conservados en el recap. */
  preservedIds: string[];
 ...
 * los identificadores clínicos verbatim. PURA: no toca el almacén de registro.
 */
export function compactContext(
  ...
  const recapLines = [
    ...
    `Se resumieron ${elided.length} intercambio(s) anterior(es) para no exceder el contexto del ` +
      `modelo. Los datos del expediente en el servidor NO se modificaron.`,
  ];
  if (preservedIds.length > 0) {
    recapLines.push("Identificadores clínicos conservados:");
