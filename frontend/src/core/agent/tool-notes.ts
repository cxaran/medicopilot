...
// cuando la charla vieja se resume. Las notas resumen, no reemplazan: los datos clínicos se releen
// frescos del expediente cuando se necesitan (la nota registra QUÉ se consultó, no es la fuente).
...
export function buildToolUsageNote(input: {
  ...
  if (isUiSpec(input.outcome.content)) {
    return `Herramienta ${input.name}(${args}) → interfaz "${input.outcome.content.kind}" mostrada al médico.`;
  }
  return `Herramienta ${input.name}(${args}) → ${briefValue(input.outcome.content, TOOL_NOTE_RESULT_CHARS)}`;
}

/**
 * Nota INFORMATIVA de una escritura P1 RECHAZADA por el médico. Deja constancia de la decisión en
...
export function buildRejectedWriteNote(plan: {
  ...
  return (
    `Propuesta de escritura RECHAZADA por el médico (${plan.actionType} → ${plan.targetResource}): ` +
    `${plan.humanReadableSummary} No volver a proponerla salvo que el médico lo pida.`
  );
}
