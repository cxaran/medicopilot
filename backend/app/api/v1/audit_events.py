...
El vínculo con un paciente se consulta por ``entity_type``/``entity_id`` (la bitácora no tiene
una columna ``patient_id`` propia): para el rastro de un paciente se filtra por
``entity_type=patient&entity_id=<id>``.
"""
