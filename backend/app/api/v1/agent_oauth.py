...
Conecta la cuenta del médico por OAuth browser-callback PKCE (NO device-code) y
...
devuelve en claro y los tokens nunca se loguean. El arriendo interno (B4) entrega
el access token vigente a partir de este perfil.
"""
...
def _require_oauth_configured() -> tuple[str, str]:
    ...
    if not client_id or not redirect_uri:
        api_error(
            ...
            "El flujo OAuth de OpenAI no está configurado en el servidor.",
        )
...
def start_oauth(
    ...
    client_id, redirect_uri = _require_oauth_configured()
...
def complete_oauth(
    ...
    try:
        profile = exchange_code(code=payload.code, code_verifier=code_verifier)
...
def oauth_status(
    ...
    # No se descifra el perfil completo para el status; se reporta conexión y, si se
    # puede leer sin exponer tokens, la cuenta y el vencimiento.
    try:
