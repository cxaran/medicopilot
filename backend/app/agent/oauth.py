...
Patrón OpenClaw (Codex): NO device-code. El navegador del médico autoriza en el
proveedor y vuelve con un ``code`` que FastAPI intercambia (con el ``code_verifier``
PKCE guardado server-side) por un perfil ``{access, refresh, expires, account_id}``.
Ese perfil se guarda CIFRADO (Fernet, reusando ``crypto``) en la credencial del
usuario y nunca se devuelve en claro.

SEGURIDAD: este módulo NUNCA loguea access/refresh tokens, el ``code`` ni el
``code_verifier``. El intercambio/refresh HTTP es un único punto (``_post_token``)
que los tests mockean; no hay llamadas reales en pruebas.
"""
...
# completa en segundos y MedicoPilot corre como un único proceso por consultorio;
# un reinicio entre /start y /complete simplemente obliga a reiniciar el flujo
# (el navegador recibe "estado OAuth inválido"). No se persiste en la BD para no
# dejar material PKCE en reposo. Si en el futuro hay múltiples procesos, mover a
# una tabla efímera o a Redis con el mismo contrato.
...
def _profile_from_token(token_response: dict[str, Any], *, fallback: dict[str, Any] | None = None) -> dict[str, Any]:
...
def exchange_code(*, code: str, code_verifier: str) -> dict[str, Any]:
    ...
    payload = {
        ...
        "redirect_uri": settings.openai_oauth_redirect_uri or "",
    }
