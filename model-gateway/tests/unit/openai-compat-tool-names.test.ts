...
// llevan punto ("clinical.search_patients", "ui.render_form"); el cable debe ir saneado y
...
const userMessage: CanonicalMessage[] = [
  { role: "user", content: [{ type: "text", text: "Busca al paciente" }] }
];

const tools: ModelToolDefinition[] = [
  {
    name: "clinical.search_patients",
    description: "Busca pacientes",
...
describe("saneo de nombres de tool (helpers)", () => {
  it("sanea el punto y trunca a 64 (patrón de OpenAI/DeepSeek)", () => {
    expect(sanitizeOACToolName("clinical.search_patients")).toBe("clinical_search_patients");
  ...
  it("buildOACToolNameMap permite el round-trip saneado -> original", () => {
    const wire: OACTool[] = [
      { type: "function", function: { name: "clinical.search_patients", description: "d", parameters: {} } }
    ];
    const map = buildOACToolNameMap(wire);
    expect(map["clinical_search_patients"]).toBe("clinical.search_patients");
  });
...
describe("runOpenAICompatChat: nombres saneados en el cable, originales hacia el cliente", () => {
  ...
  it("declara tools saneadas, revierte el nombre en tool_call.ready y conserva originales en la continuación", async () => {
          ...
          choices: [
            {
              delta: {
                tool_calls: [
                  { index: 0, id: "call_1", function: { name: "clinical_search_patients", arguments: "{}" } },
    ...
    expect(event.call.name).toBe("clinical.search_patients");
