...
const tools: ModelToolDefinition[] = [
  { name: "clinical.list_patients", description: "Lista pacientes", inputSchema: { type: "object" }, strict: false }
];
...
describe("startTurn chat_completions", () => {
  ...
  it("relay de tool call: emite tool_call.ready y reanuda con el resultado", async () => {
    const { adapter, calls } = adapterWith([
      sseResponse([
        JSON.stringify({ choices: [{ delta: { tool_calls: [{ index: 0, id: "call_1", function: { name: "clinical.list_patients", arguments: '{"limit":5}' } }] } }] }),
